...
/*      a. LENGTH, LENGTHB  : 길이 구함
         ...
         g. CONCAT                : 문자열 합쳐줌
         h. REPLACE               : 문자열 바꿔줌
         함수명(값1, 값2, . . .), 값에는 리터럴이나 컬럼명을 넣어주면 됨
*/
-- 1.1.3 날짜 처리 함수
...
/*
    ...
    (단, 1일 3끼를 먹는다고 한다.)
*/
SELECT SYSDATE "입대일자", ADD_MONTHS(SYSDATE, 18) "제대일자", ((ADD_MONTHS(SYSDATE, 18))-SYSDATE)*3 "짬밥수" FROM DUAL;
...
--	ex) 노옹철	no_hc
--	ex) 정중하	jung_jh
SELECT EMP_NAME "이름", SUBSTR(EMAIL, 1, INSTR(EMAIL,'@')-1) "아이디", INSTR(EMAIL, '@') "@위치"
FROM EMPLOYEE;
...
'19'||SUBSTR(EMP_NO,1,2)||'년'||SUBSTR(EMP_NO,3,2)||'월'
||SUBSTR(EMP_NO,5,2)||'일' "생년월일",
FLOOR(FLOOR(SYSDATE - TO_DATE(SUBSTR(EMP_NO,1,6)))/365)"나이(만)",
...
-- CASE 함수
-- 사용법 : CASE WHEN 변하는 값 = 조건값 THEN 참일때 값 WHEN 변하는 값 = 조건값 THEN 참일때 값 ELSE 모두 아닐때 값 END
...
SELECT DEPT_CODE "부서코드", SUM(SALARY) "그룹별 급여의 합계", FLOOR(AVG(SALARY)) "그룹별 급여의 평균", COUNT(SALARY) "인원수"
FROM EMPLOYEE
GROUP BY DEPT_CODE
ORDER BY 1 ASC;
...
-- BONUS 컬럼의 값이 존재한다면 그 행을 1로 카운팅, 보너스를 지급받은 사원이 없는 부서도 있음을 화인
...
SELECT JOB_CODE, COUNT(*), FLOOR(AVG(SALARY)) FROM EMPLOYEE
...
--COUNT (EXTRACT(YEAR FROM HIRE_DATE)) "입사년도별 인원수"
WHERE JOB_CODE NOT LIKE 'J1'
...
FLOOR(AVG(SALARY)), SUM(SALARY), COUNT(SALARY) FROM EMPLOYEE
...
-- 부서내 성별 인원수를 구하시오
SELECT DEPT_CODE,
DECODE(SUBSTR(EMP_NO,8,1), '1', '남', '2', '여', '3', '남', '4', '여', '무')"성별", COUNT(*)
FROM EMPLOYEE
GROUP BY DEPT_CODE,
DECODE(SUBSTR(EMP_NO,8,1), '1', '남', '2', '여', '3', '남', '4', '여', '무')
ORDER BY 1 ASC;

-- SELECT 문에서 조건을 걸때에는 WHERE을 쓰지만
SELECT * FROM EMPLOYEE
WHERE SALARY >= 5000000;
-- GROUP BY 한 결과값에 조건을 걸때에는 HAVING을 쓴다.
...
SELECT NVL(MANAGER_ID,'없음'), COUNT(*) FROM EMPLOYEE
GROUP BY MANAGER_ID
HAVING COUNT(*)>=2
ORDER BY 1 ASC;
