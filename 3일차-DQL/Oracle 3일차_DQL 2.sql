...
-- 특정 컬럼이나 특정 행을 조회할 수 있음 (WHERE 조건절)
...
SELECT EMP_NAME "이름", SALARY*12 AS 연봉, SALARY*12+SALARY*NVL(BONUS,0) "총수령액", 
...
SELECT EMP_NAME AS 이름, SALARY "월급", NVL(BONUS, 0) AS "보너스율" FROM EMPLOYEE WHERE (SYSDATE - HIRE_DATE) > 7300;
...
-- 최종 실습 문제
-- 문제1. 
-- 근속년수가 오늘 기준으로  5년 이상, 10년 이하인 직원의 이름,주민번호,급여,입사일을 검색하여라
...
--(퇴사 여부 : ENT_YN)
SELECT EMP_NAME "이름", DEPT_CODE "부서코드", HIRE_DATE "고용일", ROUND(ENT_DATE - HIRE_DATE) "근무기간" , ENT_DATE "퇴직일" FROM EMPLOYEE
WHERE ENT_YN = 'Y';
-- 문제3.
...
SELECT EMP_NAME "이름", FLOOR(FLOOR((SYSDATE-HIRE_DATE)/1000)*0.1*SALARY) "특별보너스" FROM EMPLOYEE
...
-- 실습1
-- 관리자(MANAGER_ID도 없고 부서 배치(DEPT_CODE)도 받지 않은 직원의 이름 조회하시오
