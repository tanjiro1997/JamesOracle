...
-- 왜 뒤죽박죽 나오지? 순서가 매겨진 후 정렬하니까 뒤죽박죽이지
...
(SELECT DEPT_CODE, DEPT_TITLE, FLOOR(AVG(SALARY)) "AVG_SAL" FROM EMPLOYEE
LEFT OUTER JOIN DEPARTMENT ON DEPT_CODE = DEPT_ID
GROUP BY DEPT_CODE, DEPT_TITLE ORDER BY 3 DESC) E)
...
FROM EMPLOYEE) WHERE 순위 BETWEEN 1 AND 10;
