...
BEGIN
    ...
    WHERE EMP_ID = '&EMP_ID';
    DBMS_OUTPUT.PUT_LINE('사원번호 : ' || VEMPID || ', 이름 :' ||
    VENAME || ', 부서코드 : ' || VDCODE || ', 부서명 : ' || VDTITLE);
END;
/
...
INSERT INTO EMPLOEE(EMP_ID, EMP_NAME, EMP_NO, PHONE, JOB_CODE, SAL_LEVEL)
...
BEGIN
    ...
    FROM EMPLOYEE
    WHERE EMP_ID = (&EMP_ID);
    EMP_INFO.SALARY := EMP_INFO.SALARY / 10000;
    IF (EMP_INFO.SALARY >= 500)
    THEN SLV := 'A';
    ELSIF (EMP_INFO.SALARY BETWEEN 400 AND 499)
    THEN SLV := 'B';
    ELSIF (EMP_INFO.SALARY BETWEEN 300 AND 399)
    THEN SLV := 'C';
    ELSIF (EMP_INFO.SALARY BETWEEN 200 AND 299)
    THEN SLV := 'D';
    ELSIF (EMP_INFO.SALARY BETWEEN 100 AND 199)
    THEN SLV := 'E';
    ELSE SLV := 'F';
    END IF;
    ...
    DBMS_OUTPUT.PUT_LINE('사번 : ' || EMP_INFO.EMP_ID);
    DBMS_OUTPUT.PUT_LINE('이름 : ' || EMP_INFO.EMP_NAME);
    DBMS_OUTPUT.PUT_LINE('급여 : ' || EMP_INFO.SALARY || '만원');
